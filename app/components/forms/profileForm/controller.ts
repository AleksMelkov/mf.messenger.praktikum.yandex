...
export const profileFormController:ControllerType = {
  ...
  mount() {
    ...
    eventBus.on(
      GLOBAL_EVENTS.SAVE_PROFILE,
      profileFormController.methods.submitProfile.bind(profileFormController),
    );
  ...
  methods: {
    ...
    getUserInfo() {
      ...
      Object.keys(store.value.user).forEach((key) => {
        ...
        const element = getArrayElement(profileFormController.elements, 'header', key);
        if (element.length > 0) { element[0].placeholder = store.value.user[key]; }
      });
    },
  },
  events: [
