...
export default class Profile extends Page {
    ...
    getData(): Promise<unknown> {
      const { user } = this.baseStore.value;
      if (!user.email) {
    ...
    protected profileReturnInit() {
      ...
      this.addingElement(
        ELEMENTS.PROFILE_RETURN,
        Button,
        CONTROLLERS.PROFILE_RETURN,
        profileReturnTmpl,
      );
    ...
    protected profileAvatarInit() {
      ...
      if (input instanceof HTMLInputElement) {
        input.addEventListener('change', (event) => {
          if (changeAvatarComponent.methods) { changeAvatarComponent.methods.changeAvatar(event); }
        });
      }
    }
    ...
    protected changeProfileInit() {
      ...
      this.addingElement(
        ELEMENTS.CHANGE_PROFILE,
        Button,
        CONTROLLERS.CHANGE_PROFILE,
        changeProfileTmpl,
      );
